...
export default class Trail {
  constructor(x, y, maxLength) {
    ...
    for (let i = maxLength; i >= 0; i -= 2) this.push(x - i, y, 1);
  }

  push(x, y, facing) {
    const last = this.points[this.points.length - 1];
    if (last) this.total += Math.hypot(x - last.x, y - last.y);
    this.points.push({ x, y, d: this.total, facing });
  ...
  record(x, y, facing) {
    ...
    this.push(x, y, facing);
  }

  sample(distBack) { // returns {x, y, facing}
    ...
    for (let i = pts.length - 1; i > 0; i--) {
      ...
      if (a.d <= target) {
        ...
        return { x: a.x + (b.x - a.x) * t, y: a.y + (b.y - a.y) * t, facing: b.facing };
      }
    }
    return pts[0];
  }
