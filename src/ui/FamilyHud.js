...
export default class FamilyHud {
  constructor(scene, family) {
    ...
    this.icons = family.members.map((m, i) => {
      const icon = scene.add.image(6 + i * 12, 6, `${m.memberName}_front`).setOrigin(0).setScale(0.5 * ART_SCALE)
  ...
  update() {
    ...
    this.icons.forEach(({ icon, x }) => { icon.setVisible(!hide); if (hide) x.setVisible(false); });
    if (hide) return;
    ...
    this.family.members.forEach((m, i) => {
      ...
      icon.setTexture(salted ? 'salt' : `${m.memberName}_front`);
